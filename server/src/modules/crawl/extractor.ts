...
export interface ExtractedCandidate {
  ...
  reason?: string;
  /** 作品主页/个人网站/社媒链接（名单里常有，用于建联时跳转查看作品） */
  website?: string;
...
{"name":"对象名","type":"creatorKol","region":"","booth":"Table S-12","activityTime":"","followerScale":"","website":"","reason":"","rawSnippet":"原文出处片段"}
- activityTime：到场/签售/现场时段（如"7/3 14:00 签售"），名单里有才填。
- website：该对象的作品主页/个人网站/社媒链接（原文里常标 Website: 或直接是 http 链接），原样填完整 URL，没有则留空。绝不编造链接。
若没抽到任何对象，输出 []。`;
...
export async function extractCandidates(
  llm: LlmClient,
  rawText: string,
  ctx: { sourceName?: string; eventShort?: string; maxChunks?: number; startChunk?: number },
): Promise<{
  candidates: ExtractedCandidate[];
  usage: Record<string, number>;
  model: string;
  truncated: boolean;
  /** 下次应从第几段继续；已抽到末尾则回绕为 0 */
  nextOffset: number;
  /** 名单总段数 */
  totalChunks: number;
}> {
  ...
  // 断点续抓：从 startChunk 开始，本次只处理 maxChunks 段，剩下的下次继续。
  const maxChunks = ctx.maxChunks ?? 8;
  const start = Math.min(Math.max(0, ctx.startChunk ?? 0), allChunks.length);
...
/** 只接受 http/https 绝对链接，防 LLM 编造相对路径/占位 */
